<?php
	...
	It's very simple and small APRX statictics generator in PHP.
	It's parted to smaller files and they will work independent from each other
	(but you always need chgif.php). This script may have a lot of bugs, problems
	and it's written in very non-efficient way without a lot of good programming
	...
	******************************************************************************/
	...
	if ( isset( $_GET['getcall'] ) && ( $_GET['getcall'] != "" ) )
	{
		...
		while ($lines > 0)														// read line by line but starting from the newest frame!
		{
			...
			if ( 
				( strpos( $line, $callraw." R " ) !== false ) OR ( strpos ( $line, $callraw." d " ) !== false ) )
			{
				frameparse( $line );
			}
...
<html>
	...
	<body>
		...
		?>
		<center>
			<font size="20"><b>APRX statistics</b></font><h2>for interface <font color="red"><b><?php echo $call; ?></b></font> - station's details</h2> <a href="chgif.php?chgif=1">Change interface</a>
			<br>
			<br>
			<b>Show:</b> <a href="summary.php">Summary (main)</a> - <a href="frames.php">RAW frames from specified station</a> - <a href="details.php">Details of a specified station</a>
			<br>
			<br>
			<hr>
		</center>
			...
			if ( isset( $_GET['getcall'] ) && ( $_GET['getcall'] != "" ) )
			{
				...
				else
				{
					...
					if ( $posframefound )
					{
						...
						if ( $mice ) echo 'Mic-E compressed frame';
						else echo 'Uncompressed frame';
...
<html>
	...
	<body>
		...
		<center>
			<font size="20"><b>Statystyki APRX</b></font><h2>dla interfejsu <font color="red"><b><?php echo $call; ?></b></font> - szczegóły stacji</h2> <a href="chgif.php?chgif=1">Zmień interfejs</a>
			<br>
			<br>
			<b>Pokaż:</b> <a href="summary.php">Podsumowanie (główna)</a> - <a href="frames.php">Surowe ramki wybranej stacji</a> - <a href="details.php">Szczegóły wybranej stacji</a>
			<br>
			<br>
			<hr>
		</center>
			...
			if ( isset( $_GET['getcall'] ) && ( $_GET['getcall'] != "" ) )
			{
				...
				else
				{
					...
					if ( $posframefound )
					{
						...
						if ( $mice ) echo 'Ramka skompresowana Mic-E';
						else echo 'Ramka nieskompresowana';
		...
		<center>
			<a
				href	= "https://github.com/sq8vps/aprx-simplewebstat"
				target	= "_blank"
			>APRX Simple Webstat version <?php echo $asw_version; ?></a> by Peter SQ8VPS and Alfredo IZ7BOJ
		</center>
